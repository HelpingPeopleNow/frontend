...
export default function ConversationDetailPage({ id }: Props) {
  const { t } = useLanguage();
  document.title = `Admin - Conversation #${id} | Helping People`;
  ...
  useEffect(() => {
    Promise.all([
      getEntity<Record<string, unknown>>('conversations', id),
      listEntities<Message>('messages', 1000),
    ])
      .then(([c, msgs]) => {
        setConv(c);
        setMessages(msgs.filter(m => m.conversation_id === id).sort(
      ...
      .catch(err => {
        ...
        logError('admin', `get conversation ${id} failed: ${msg}`);
        setError(msg);
        setLoading(false);
      });
  }, [id]);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteEntity('conversations', id);
      route('/admin/conversations');
    } catch (err) {
      const msg = err instanceof ApiError ? err.message : err instanceof Error ? err.message : 'request failed';
      logError('admin', `delete conversations/${id} failed: ${msg}`);
    ...
    <AppShell currentPath="/admin" title="Admin">
      <div class="admin-page">
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-3)', marginBottom: 'var(--sp-5)' }}>
          <button class="btn btn-ghost" onClick={() => route('/admin/conversations')} style={{ fontSize: 'var(--text-lg)' }}>←</button>
          <h2 class="section-title" style={{ margin: 0 }}>💬 Conversation #{id}</h2>
          <div style={{ flex: 1 }} />
            ...
            {Object.entries(conv).map(([key, val]) => (
              <div class="entity-field" key={key}>
                ...
                <div class="entity-field-value">
                  {val === null ? <span class="text-muted">null</span> : String(val)}
                </div>
              </div>
            ))}
          </div>
        ...
        {messages.length === 0 ? (
          ...
          <div class="admin-transcript">
            {messages.map(m => (
              <div class={`dm-bubble dm-${m.role === 'user' ? 'out' : 'in'}`} key={m.id}>
                <div class="dm-bubble-meta">
                  <span class="dm-bubble-sender">{m.role}</span>
                  <span class="dm-bubble-time">{new Date(m.created_at).toLocaleString()}</span>
                </div>
                <div class="dm-bubble-body">{m.content}</div>
              </div>
            ))}
          </div>
        )}
